...
async function seed() {
  ...
  try {
    ...
    const dealersService = app.get(DealersService);
    ...
    await ordersService.updateStatus(multiItemOrder.id, OrderStatus.InTransit);
    ...
    console.table([
      ...
      { label: 'Multi-item order', id: multiItemOrder.id, status: OrderStatus.InTransit, dealer: multiItemOrder.assignedDealerName, total: multiItemOrder.totalCost },
